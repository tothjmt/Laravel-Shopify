<?php namespace tothjmt\Shopify;

/**
 * Class API
 * @package tothjmt\Shopify
 */
class API
{
	private $_API = array();
	...
	public function installURL($data = array())
	{
		...
		return 'https://' . $this->_API['SHOP_DOMAIN'] . '/admin/oauth/authorize?client_id=' . $this->_API['API_KEY'] . '&scope=' . implode(',', $data['permissions']) . (!empty($data['redirect_uri']) ? '&redirect_uri=' . urlencode($data['redirect_uri']) : '');
	}
    ...
    public function call($userData = array(), $verifyData = TRUE)
    {
        ...
        $options = array(
            ...
            CURLOPT_ENCODING        => '',
            CURLOPT_USERAGENT       => 'Laravel Shopify API Wrapper',
